...
类加载：虚拟机把描述类的数据 从Class 文件加载到内存，并对数据进行校验、转换、解析和初始化，最终形成可被虚拟机直接使用的 Java 类型，这就是虚拟机的类加载机制

Java 类加载机制（开始的顺序）：加载、验证、准备、解析（顺序不一定确定）、初始化、使用、卸载

### 类加载(初始化)时机

加载的时机（JVM 只严格规范了有且仅有五种情况下，必须立即对类进行初始化，加载肯定发生在这之前）

* 遇到 `new getstatic putstatic invokestatic` 这四条字节码指令时，如果类还没有进行初始化，则需要先触发其初始化。生成这四条指令最常见的场景是：
  * 使用 new 实例化对象的时候
  * 读取或设置一个类的静态字段的时候（被 final 修饰、已在编译期把结果放入常量池的除外）
  * 调用一个类的静态方法的时候
* 使用 `java.lang.reflect` 包的方法对类进行反射调用的时候，如果类还未初始化，则会先触发其初始化
* 当初始化一个类时，如果其父类还未初始化，则需要先触发其父类的初始化
* 当虚拟机启动时，需要制定一个要执行的主类（包含main方法的那个），虚拟机需要先初始化这个主类
* 当使用 JDK1.7 动态语言支持的时候，如果一个 `java.lang.invoke.MethodHandle` 实例最后的解析结果`REF_getStatic REF_putStatic REF_invokeStatic` 的方法句柄，并且这个方法句柄对应的类还未初始化，则需要先触发其初始化

### 类加载过程

* 加载：在类加载阶段，主要完成以下三件事
  * 通过一个类的全限定名 来获取 定义此类的二进制字节流
  * 将字节流的所代表的静态存储结构，转化为方法去的运行时数据结构
  * 在内存中生成一个 代表这个类的 `java.lang.Class` 对象，作为方法区中这个类的各种数据的访问入口
* 验证：确保 class 文件的字节流中包含的信息符合当前虚拟机的要求，并且不会危害虚拟机自身的安全
* 准备：正式为 类变量分配内存 并且设置 类变量初始值的阶段，这些变量所使用的内存，都在方法区分配
  * 这里的变量仅仅包括类变量（被static修饰）
  * 通常情况下初始值是 零值（被 final 修饰的就会直接赋定义的值）
* 解析：虚拟机将常量池中的符号引用，替换为直接引用的过程
  * 符号引用：使用一组符号来描述所引用的对象，可以是任何形式的字面量，只要能无歧义的定位到目标即可；与虚拟机的内存布局无关
  * 直接引用：直接指向目标的 指针、相对偏移量、能间接定位到目标的句柄。和虚拟机的内存布局相关
* 初始化：执行类构造器 `<clinit>()` 方法的过程
  * 顺序就是源文件的中的顺序。静态代码块只能访问到定义在其之前的变量，定义其之后的仅可以赋值，不能够访问
  * 父类的 `<clinit>()` 方法先执行，意味着父类中的静态代码块 要 优先于子类中的赋值操作
...
* 编译：创建源文件之后，程序要先被 JVM 中的 java编译器 进行编译为 `.class` 文件

  * java 编译一个类时，如果这个类所依赖的类还没有被编译，编译器就会自动地先编译这个所依赖的类，然后引用
  * 编译后的字节码文件格式主要分为两部分：常量池 和 方法字节码
    * 常量池：代码中出现过的 字面量（常量、类名、符号变量 等）；符号引用
    * 方法字节码：各个方法的字节码

* 运行：主要分为 类的加载 和 类的执行。**JVM 主要在程序的第一次运行时主动使用类的时候，才会去加载。**（JVM 不是在运行时把所有类都加载进来，而是在用到，不得不加载的时候，才加载进来，而且只加载一次）

  * 在类的路径下找到编译好的字节码文件，系统启动一个 JVM 线程，JVM线程从 `classpath` 路径下找到一个 `.class` 后缀的二进制文件，将 二进制文件内的 **类信息加载到运行时数据区的方法区中**

  * JVM 找到 main 方法主函数的入口，只有一个指向当前类常量池的指针。如果在常量池中用到了其他对象的符号引用，而且方法指令进行了对象初始化，方法区中还没有被建立对象的类信息。JVM 就会立即加载被引用到的类，并以类指针替换掉 当前类常量池中的 符号引用

  * 加载完之后，就会在堆上为实例分配内存

  * 调用方法时，JVM 根据栈中的 animal引用找到Animal对象，然后根据Animal对象持有的引用（访问定位，句柄和直接指针），定位到方法区中 Animal 类的类型信息方法表，获取函数的字节码地址，开始运行函数

    ![image-20200830212609310](C:\Users\hg\AppData\Roaming\Typora\typora-user-images\image-20200830212609310.png)
...
目的：建立连接；同步连接双方的序列号和确认号；交换 TCP 窗口大小信息

第一次握手（**此报文段不能携带数据，但是要消耗掉一个序号**）
...
* 服务器收到 syn 包，确认客户的 seq（ack=j+1）并发送自己的 SYN 包，进入 SYN_RCVD 状态
* Client确认自己发送、接受正常，对方发送、接收正常；Server确认对方发送、自己接受正常

第三次握手（**此报文段可以选择携带数据，不携带数据则不消耗序列号**）
 ...
 **半连接队列**：服务器第一次收到客户端的 SYN 之后，就会处于 SYN_RCVD 状态，此时两者还未完全建立连接，服务器会把此状态下的连接放到一个队列里，把这种队列称为

**全连接队列**：完成三次握手，建立起连接的就会放在全连接队列里。如果队列满了，可能会出现丢包现象

### SYN攻击

服务端的资源 是在 二次握手时 分配的；客户端的资源 是在 三次握手时 分配的。

服务端收到SYN攻击，是Client在短时间内伪造大量不存在的 ip 地址，并向 Server 不断地发送 SYN 包，Server 则回复确认包，并等待 Client 确认。由于源地址不存在，因此 Server 会不断重发直至超时，这些伪造的 SYN包 将长时间占用 半连接队列，导致正常的SYN请求因为队列满而被丢弃。

### 2MSL

MSL：TCP报文的最大生存时间，是任何报文段被丢弃前在网络内的最长时间

当 TCP 执行一个主动关闭，并发回最后一个 ACK，该连接必须在 TIME_WAIT 状态停留的时间为 2倍的 MSL。这样可以让 TCP 再次发送最后这个 ACK，以防 ACK 丢失

为什么要等待 2MSL

* 保证客户端的最后一个 ACK 请求能够达到服务端

  如果客户端 发送完 ACK 之后就立即关闭，一旦这个 ACK 丢失，服务器就无法进入正常关闭状态，会一直等待客户端的确认；如果等待 2MSL，客户端就能收到 服务端超时重传的 FIN+ACK 报文段

* 防止 已失效的连接请求报文段 出现在本连接中

  客户端发送完最后一个 ACK 之后等待 2MSL，就可

  以使本连接持续时间内所产生的所有报文段都从网络中消失，使下一个连接中不会出现旧连接请求中的报文段

### 四次挥手

![image-20200902140311665](C:\Users\hg\AppData\Roaming\Typora\typora-user-images\image-20200902140311665.png)
...
* http0.9：只有 `GET` 请求，不支持请求头信息
* http1.0：在请求中指定版本号，支持 `GET POST HEAD`
* http1.1：
  * 新增了 `OPTIONS、PUT、DELETE、TRACE、CONNECT` 五种 HTTP 请求
  * 需要指定请求头；长连接
* http2：
  * http1.x 的解析基于文本，http2 的解析基于二进制
  * 多路复用
...
* session：通过服务端记录用户的状态，**保存在服务器端**
  * 购物车场景
  * 如果浏览器禁止了 cookie，那么可以把 sessionid写入 url 中来传递
...
* 非对称加密：加密和解密是不同的密钥；公钥加密的信息只能用私钥解密，私钥加密的信息只能用公钥解密

  * 数字证书：确认客户端收到是服务端的公钥

    数字证书是一段信息，包括：签发证书的机构；加密算法；hash算法；公钥；到期时间等等

  * 数字签名：防止证书颁发的过程中被人修改

    * 将证书的内容做一个hash操作，生成固定长度的数据发送给服务端，服务端自己hash一遍，对比传过来的 hash 值，来判断内容是否被修改
    * 为了防止证书被修改，CA机构还使用私钥对hash值进行了加密

SSL 握手协议

* 客户端发送：支持的SSL协议 + 支持的加密方式 + 随机数
* 服务端发送：确认SSL协议和加密方式 + 数字证书 + 随机数
* 客户端：确认证书有效性，发送一个用公钥加密的随机数，用前三个随机数通过约定的算法生成 对话密钥
* 服务端：用自己的私钥解密随机数，然后用这三个随机数通过约定的算法生成对话密钥
