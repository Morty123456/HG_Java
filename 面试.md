...
* NIO：同步非阻塞IO
  ...
  * channel (通道)：NIO 的 所有 IO 都是从通道开始的
    ...
    * 从通带进行数据写入：创建缓冲区，填充数据，并要求通道写入数据
...
- `Exception` 类：分为运行时异常和受检查时异常
  ...
  - 受检查异常：
    ...
    - 异常处理的方式（**应该尽量捕获和处理一直如何处理的异常，抛出不知如何处理的异常**）
...
* 提供注册表：注册表是所有注册上来服务的列表。Eureka Client 在调用服务是，需要获取注册表。一般来说，注册表会缓存下来，如果缓存失效，就会获取最新的注册表
...
* 管道
  ...
  * 命名管道：在内核中申请一块固定大小的缓存，程序拥有读写的权利，没有血缘关系的进程也可以进行进程间的通信
  * 特点：
    * 面向字节流
    * 生命周期和内核一样
    * 自带同步互斥机制
...
* 消息队列：在内核中创建一个队列，每个元素是一个数据报，不同的进程通过句柄去访问这个队列。
  ...
  * 特点
    ...
    * 生命周期随内核
    * 可实现双向通信
* 信号量：在内核中创建一个信号量集合（本质是个数组），数组元素都是1，使用P操作进行-1，使用V操作+1
