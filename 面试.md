...
* 校验和
  ...
  * 如何计算：将放松的数据段都当作 16 位的整数，把这些整数加起来，前面的进位补在后面，最后取反，得到校验和。

* 序列号和确认应答

  * 序列号：为了识别是否已经接收数据，以及判断是否需要接收数据（在收到数据时，不需要一点点去判断是否重复，只需要看序列号就可以判断数据是否重复）
  * 确认应答号：自己下一步应该接受的序列号（告诉发送方，接收到了哪些数据，下一次的数据从哪里发送）

* 超时重传

  * 定义：指的是在重发数据之前，等待确认应答到来的那个特定的时间间隔。如果超过了这个时间间隔还未收到确认应答，发送端就要重发数据
  * 如何计算：每次发包时，都会计算往返时间 及其 偏差。两者之和是重发超时 的时间
...
* TCP 以段为单位发送数据
  ...
  * MSS：最大消息长度。理想情况下，最大消息长度正好是IP中不会被分片处理的最大数据长度
  * 如何计算：进行三次握手时，在两端主机之间计算得出。选择两者之间较小的MSS投入使用
...
* 窗口控制与重发控制
  ...
  * 确认应答丢失：在没有使用窗口控制的时候，没有收到确认应答的数据都会被重发；使用了窗口控制之后，即使有小部分确认应答丢失，也可以通过下一个确认应答进行确认，不再进行数据重发
  * 报文段丢失：如果报文段丢失（比如 1-100 丢失），那么接收端的确认应答号就会不断的返回某个相同值（101）。**如果收到三个同样的确认应答时，则进行重发**

* 流控制

  * 具体操作：接收端主机向发送端主机通知自己可以接收数据的大小，于是发送端就i会发送不超过这个限度的数据。这个限度就是之前提到的窗口大小

* 拥塞控制

  * 慢启动：发送端为了调节要发送的数据量，定义了“拥塞窗口”的概念。再慢启动的时候，将这个窗口设置为 1，之后每收到一次 ACK，窗口值就 +1。在发送数据包时，将拥塞窗口和接收端主机通知的窗口大小作比较，选择较小的值，发送比其还小的数据量
...
* 长连接：keep-alive，减少了TCP连接的重复建立和断开造成额外的开销
* 管线化：并行发送多个请求，不必等待收到上一个请求的响应
...
* 安全性 和 资源消耗
  ...
  * HTTPS 运行在 SSL/TLS 之上的 HTTP 协议，SSL/TLS 运行在 TCP 之上，所有传输内容都经过加密，加密采用对称加密，但是对称加密的密钥用服务器的证书进行了非对称加密
    * 公有密钥：加密使用
    * 私有密钥：解密使用
