...
* 虚拟机位新生对象分配内存（分配内存大小在类加载完成后便完全确定）
  * 在 Java 堆上的内存分配方式
...
* 最终标记：修正并发标记期间发生变化的标记记录，记录最终合并到 Remember Set 中；可以并行执行
...
2. class文件：使用到了监视器，monitorenter、moniterexit
3. 执行过程中自动升级
4. 汇编层面：lock cmpxchg
...
在没有多线程竞争的前提下，减少传统的 重量级锁使用操作系统互斥量 产生的性能消耗
...
* 服务器收到 syn 包，确认客户的 SYN（ack=j+1）并发送自己的 SYN 包，进入 SYN_RECV 状态
